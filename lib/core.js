...
function generate(opts) {
    ...
    return Bluebird.map(opts.dimensions, function (dimensions) {
        ...
        if(opts.dest.endsWith('phtml')){
            var regEx = /url\(["']?((..\/)*)(.*?)["']?\)/gi;
            // Replace relative paths with magento skin URLs
            criticalCSS = criticalCSS.replace(regEx, function ($0, $1, $2, $3) {

                var fullString = $0;
                var preString = $1;
                var pathString = $3;
                
                // Check if pathString is a webaddress, if it is, don't convert to skinUrl
                if (/^(f|ht)tps?:\/\//i.test(pathString) || /^\/\//i.test(pathString)) {
                   return fullString;
                }

                fullString = fullString.replace($1, "");
                fullString = fullString.replace($3, "<?php echo $this->getSkinUrl('" + $3 + "', array('_secure'=>true)); ?>");
                return fullString
            });
        }
